...
4、在SpringBoot的yml配置文件中，配置连接MySQL数据库
...
5、创建User和Work的Entity
![创建Entity](./images/image007.png)
```java
...
7、创建UserWorkMapper接口类，进行查询处理。
![Mapper接口类](./images/image010.png)
```java
...
public interface UserWorkMapper extends BaseMapper<User> {
    // 查（多表关联:1对1）：查询所有用户以及职位信息
    List<User> getUserWorkAll();

    // 查（多表关联:1对多）：查询所有用户以及职位信息
    List<User> getUserWorkList();
}
```

8、创建Controller执行查询数据（UserWorkController.java）
![Controller](./images/image011.png)
```java
package com.koguro.mybatisplus.controller;

import com.koguro.mybatisplus.entities.User;
import com.koguro.mybatisplus.mapper.UserWorkMapper;
...
public class UserWorkController {
    ...
    UserWorkMapper uwMapper;

    // 多表关联:1对1
    // 获取User的所有用户以及职位信息
    @GetMapping(value = "/getUWAll")
    public String getUserWorkAll(Model model) {
        // 调用UserWorkMapper中getUserWorkAll()方法
        List<User> awResults = uwMapper.getUserWorkAll();
        // 将得到查询结果集合放在请求域中
        model.addAttribute("awResults", awResults);
        // 跳转到信息一览页面
        return "/userWorkPage";
    }
    ...
    // 多表关联:1对多
    // 获取User的所有用户以及职位信息
    @GetMapping(value = "/getUWList")
    public String getUserWorkList(Model model) {
        // 调用UserWorkMapper中getUserWorkList()方法
        List<User> awResults = uwMapper.getUserWorkList();
        // 将得到查询结果集合放在请求域中
        model.addAttribute("awResults", awResults);
        // 跳转到信息一览页面
        return "/userWorkPage";
...
9、创建一个映射文件，用于实现SQL文（UserWorkMapper.xml）
...
<mapper namespace="com.koguro.mybatisplus.mapper.UserWorkMapper">
    ...
    <!-- 关联查询UserWork信息方法 -->
    <select id="getUserWorkAll" resultMap="userWorkMap">
        select u.*, w.* from tbl_user u LEFT JOIN tbl_work w on u.wid = w.id
    </select>

    <select id="getUserWorkList" resultMap="userWorkList">
        select u.*, w.* from tbl_user u LEFT JOIN tbl_work w on u.wid = w.id
    </select>
</mapper>
```

10、修改数据的配置文件「application.yml」文件，加载mybatis映射文件
...
11、创建查询结果显示画面（userWorkPage.html）
